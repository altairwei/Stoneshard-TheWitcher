...
namespace TheWitcher
{
    public static partial class TableUtils
    {
        ...
        private static readonly Dictionary<ArmorTier, string> ArmorTierMap = new Dictionary<ArmorTier, string>
        {
            { ArmorTier.Tier1, "1" },
            { ArmorTier.Tier2, "2" },
            { ArmorTier.Tier3, "3" },
        ...
        private static readonly Dictionary<ArmorTags, string> ArmorTagsMap = new Dictionary<ArmorTags, string>
        {
            { ArmorTags.aldor, "aldor" },
            { ArmorTags.fjall, "fjall" },
            { ArmorTags.elven, "elven" },
            { ArmorTags.special, "special" },
            { ArmorTags.unique, "unique" },
            { ArmorTags.skadia, "skadia" },
            { ArmorTags.nistra, "nistra" },
            { ArmorTags.WIP, "WIP" },
            { ArmorTags.magic, "magic" },
            { ArmorTags.specialexc, "special exc" }
        };

        private static readonly Dictionary<ArmorHook, string> ArmorHookMap = new Dictionary<ArmorHook, string>
        {
            { ArmorHook.SHIELDS, "SHIELDS" },
            { ArmorHook.HELMETS, "HELMETS" },
            { ArmorHook.CHESTPIECES, "CHESTPIECES" },
            { ArmorHook.GLOVES, "GLOVES" },
            { ArmorHook.BOOTS, "BOOTS" },
            { ArmorHook.BELTS, "BELTS" },
            { ArmorHook.RINGS, "RINGS" },
            { ArmorHook.NECKLACES, "NECKLACES" },
            { ArmorHook.CLOAKS, "CLOAKS" }
        };
        public static void InjectTableArmor(
            ...
            bool IsOpen = false,
            short? visorSwitch = null, // type unknown
            ...
            )
        {
            ...
            // Prepare line
            string newline = $"{name};{GetEnumValue(Tier, ArmorTierMap)};{id};{Slot};{Class};{rarity};{Mat};{Price};{Markup};{MaxDuration};{DEF};;{PRR};{Block_Power};{Block_Recovery};{EVS};{Crit_Avoid};{FMB};{Hit_Chance};{Weapon_Damage};{Armor_Piercing};{Armor_Damage};{CRT};{CRTD};{CTA};{Damage_Received};{Fortitude};;{MP};{MP_Restoration};{Abilities_Energy_Cost};{Skills_Energy_Cost};{Spells_Energy_Cost};{Magic_Power};{Miscast_Chance};{Miracle_Chance};{Miracle_Power};{Cooldown_Reduction};;{VSN};{max_hp};{Health_Restoration};{Healing_Received};{Lifesteal};{Manasteal};{Bonus_Range};{Received_XP};{Damage_Returned};;{Bleeding_Resistance};{Knockback_Resistance};{Stun_Resistance};{Pain_Resistance};{Fatigue_Gain};;{Physical_Resistance};{Nature_Resistance};{Magic_Resistance};;{Slashing_Resistance};{Piercing_Resistance};{Blunt_Resistance};{Rending_Resistance};{Fire_Resistance};{Shock_Resistance};{Poison_Resistance};{Caustic_Resistance};{Frost_Resistance};{Arcane_Resistance};{Unholy_Resistance};{Sacred_Resistance};{Psionic_Resistance};;{Pyromantic_Power};{Geomantic_Power};{Venomantic_Power};{Electromantic_Power};{Cryomantic_Power};{Arcanistic_Power};{Astromantic_Power};{Psimantic_Power};;{GetEnumValue(tags, ArmorTagsMap)};{(fireproof ? "1" : "")};{(IsOpen ? "1" : "")};{visorSwitch};{(NoDrop ? "1" : "")};{fragment_cloth01};{fragment_cloth02};{fragment_cloth03};{fragment_cloth04};{fragment_leather01};{fragment_leather02};{fragment_leather03};{fragment_leather04};{fragment_metal01};{fragment_metal02};{fragment_metal03};{fragment_metal04};{fragment_gold};";

            // Find Meta Category in table
            string hookStr = "";

            if (hook == ArmorHook.CHESTPIECES)
                hookStr = "// " + GetEnumValue(hook, ArmorHookMap); // Necessary workaround for chestpieces as the devs messed up
            else
                hookStr = "[ " + GetEnumValue(hook, ArmorHookMap) + " ]";
